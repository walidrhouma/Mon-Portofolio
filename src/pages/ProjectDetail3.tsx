  ...
  return (
    <div className="min-h-screen bg-base-900 text-slate-200">
      <div className="container-page py-12 lg:py-16 space-y-12">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="p-5 rounded-2xl bg-base-700/60 border border-white/5 backdrop-blur-sm space-y-1">
            <h3 className="text-lg font-bold text-cyber-blue">BeeSafe</h3>
            <p className="text-xs text-slate-400">Contexte Startup Assurance</p>
          </div>
          <div className="p-5 rounded-2xl bg-base-700/60 border border-white/5 backdrop-blur-sm space-y-1">
            <h3 className="text-lg font-bold text-cyber-emerald">Debian 12 + Win11</h3>
            <p className="text-xs text-slate-400">Apache 2.4, MariaDB 15.1, Bind9</p>
          </div>
          <div className="p-5 rounded-2xl bg-base-700/60 border border-white/5 backdrop-blur-sm space-y-1">
            <h3 className="text-lg font-bold text-cyber-blue">Architecture 3-Tiers</h3>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <div className="bg-base-700/60 border border-white/5 rounded-2xl p-6 md:p-8 backdrop-blur-sm space-y-6">
              ...
              <div className="pt-2">
                <div className="p-4 rounded-xl bg-base-900/40 border border-white/5 space-y-3">
                  <div className="flex justify-between items-center">
                    <h3 className="text-sm font-semibold text-cyber-blue">Schéma d'Architecture 3-Tiers</h3>
                  ...
                  <div 
                    ...
                    className="relative group cursor-pointer overflow-hidden rounded-lg border border-white/5 bg-base-700 h-48 flex items-center justify-center"
                  >
                    <div className="text-center p-4">
                      <FileText className="w-12 h-12 text-cyber-blue mx-auto mb-2 opacity-80 group-hover:scale-110 transition-transform" />
                      <span className="text-xs text-slate-300 font-medium block">Schéma d'Architecture 3-Tiers (PDF)</span>
                      <span className="text-[11px] text-cyber-blue mt-1 inline-flex items-center gap-1"><ZoomIn className="w-3.5 h-3.5" /> Aperçu rapide</span>
                    </div>
                  </div>
                </div>
              </div>
                  ...
                  <tbody className="divide-y divide-white/5">
                    {architectureData.map((row, index) => (
                      <tr key={index} className="hover:bg-base-700/30 transition-colors">
                        <td className="py-3 px-4 font-semibold text-cyber-blue">{row.machine}</td>
                        <td className="py-3 px-4 font-medium text-slate-200">{row.os}</td>
                        <td className="py-3 px-4 font-mono font-bold text-cyber-emerald">{row.ip}</td>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-base-700/60 border border-white/5 rounded-2xl p-6 backdrop-blur-sm space-y-6">
              ...
              <div className="space-y-6">
                {deliverables.map((deliv, groupIdx) => (
                  <div key={groupIdx} className="space-y-2">
                    <h4 className="text-xs font-bold text-cyber-blue uppercase tracking-wider flex items-center gap-1.5">
                    ...
                    <div className="space-y-2 pl-2 border-l border-white/5">
                      {deliv.files.map((fileItem, fileIdx) => {
                        ...
                        return (
                          <div 
                            key={fileIdx} 
                            className="p-3 rounded-xl bg-base-900/50 border border-white/5 hover:border-cyber-blue/30 transition-all flex flex-col gap-2.5"
                          >
                            <div className="flex items-start gap-2">
                              <FileText className="w-4 h-4 text-cyber-blue shrink-0 mt-0.5" />
                              <div>
                            ...
                            <div className="grid grid-cols-2 gap-2">
                              ...
                              <a
                                ...
                                className="inline-flex items-center justify-center gap-1 px-2.5 py-1.5 rounded-lg bg-base-700 text-slate-200 hover:bg-base-600 hover:text-white transition-all text-[11px] font-medium"
                              >
                                <Eye className="w-3 h-3 text-cyber-blue" />
                                Consulter
                              </a>
                              ...
                              <button
                                ...
                                className="inline-flex items-center justify-center gap-1 px-2.5 py-1.5 rounded-lg bg-cyber-blue/10 text-cyber-blue border border-cyber-blue/20 hover:bg-cyber-blue/90 hover:text-white transition-all text-[11px] font-semibold"
                              >
            ...
            <div className="bg-base-700/60 border border-white/5 rounded-2xl p-6 backdrop-blur-sm space-y-4">
              ...
              <div className="space-y-3">
                {stackTools.map((tool, i) => (
                  <div key={i} className="p-3 rounded-xl bg-base-900/40 border border-white/5 flex items-center gap-3.5 hover:border-white/10 transition-colors">
                    <div className="p-2 rounded-lg bg-base-700/80 border border-white/5 shrink-0 flex items-center justify-center w-10 h-10">
                      <img 
                        src={tool.logo} 
                        alt={`Logo ${tool.name}`} 
                    ...
                    <div className="flex flex-col">
                      <span className="text-xs font-semibold text-cyber-blue font-mono">{tool.name}</span>
      ...
      {selectedImage && (
        ...
        >
          <div className="relative max-w-5xl w-full h-[85vh] bg-base-700 border border-white/5 rounded-2xl p-4 overflow-hidden" onClick={(e) => e.stopPropagation()}>
            <button 
              onClick={() => setSelectedImage(null)}
      ...
      <EvaluationJury
        ...
        summary={"Competences validees : installation des composantes SI et modelisation."}
        quote={"Les services DNS, Apache, SQL et les droits associes ont ete correctement mis en place et documentes. Projet valide."}
      />
      <ProjectFooterNav current={3} />
    </div>
  );
}