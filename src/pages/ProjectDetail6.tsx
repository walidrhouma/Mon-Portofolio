...
export default function ProjectDetail6() {
  ...
  return (
    <div className="min-h-screen bg-base-900 text-slate-200">
      <div className="container-page py-12 lg:py-16 space-y-12">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="p-5 rounded-2xl bg-base-700/60 border border-white/5 backdrop-blur-sm space-y-1">
            <h3 className="text-lg font-bold text-cyber-blue">OSPF IPv4 / IPv6</h3>
            <p className="text-xs text-slate-400">Routage dynamique multi-sites</p>
          </div>
          <div className="p-5 rounded-2xl bg-base-700/60 border border-white/5 backdrop-blur-sm space-y-1">
            <h3 className="text-lg font-bold text-cyber-emerald">VLANs & LACP</h3>
            <p className="text-xs text-slate-400">Segmentation & Agrégation 4 liens</p>
          </div>
          <div className="p-5 rounded-2xl bg-base-700/60 border border-white/5 backdrop-blur-sm space-y-1">
            <h3 className="text-lg font-bold text-cyber-blue">NAT & Services</h3>
            ...
            <div className="bg-base-700/60 border border-white/5 rounded-2xl p-6 md:p-8 backdrop-blur-sm space-y-6">
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-white/5 pb-4">
                ...
                <div className="flex flex-wrap gap-1 bg-base-900/80 p-1 rounded-xl border border-white/5 text-xs font-mono">
                  <button 
                    ...
                    className={`px-3 py-1.5 rounded-lg transition-all ${activeTab === 'topology' ? 'bg-cyber-blue/20 text-cyber-blue border border-cyber-blue/30' : 'text-slate-400 hover:text-white'}`}
                  >
                    Topologie
                  </button>
                  <button 
                    onClick={() => setActiveTab('services')}
                    className={`px-3 py-1.5 rounded-lg transition-all ${activeTab === 'services' ? 'bg-cyber-blue/20 text-cyber-blue border border-cyber-blue/30' : 'text-slate-400 hover:text-white'}`}
                  >
                    DNS & NAT
                  </button>
                  <button 
                    onClick={() => setActiveTab('switching')}
                    className={`px-3 py-1.5 rounded-lg transition-all ${activeTab === 'switching' ? 'bg-cyber-blue/20 text-cyber-blue border border-cyber-blue/30' : 'text-slate-400 hover:text-white'}`}
                  >
                    Switching & ACLs
                  </button>
                  <button 
                    onClick={() => setActiveTab('cli')}
                    className={`px-3 py-1.5 rounded-lg transition-all ${activeTab === 'cli' ? 'bg-cyber-blue/20 text-cyber-blue border border-cyber-blue/30' : 'text-slate-400 hover:text-white'}`}
                  >
                    Commandes CLI
                  </button>
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {galleryTabs[activeTab as keyof typeof galleryTabs].map((img, idx) => (
                  <div key={idx} className={`p-4 rounded-xl bg-base-900/60 border border-white/5 space-y-3 ${galleryTabs[activeTab as keyof typeof galleryTabs].length === 1 ? 'md:col-span-2' : ''}`}>
                  ...
                  <div className="space-y-1 text-[11px]">
                    <div className="flex items-center justify-between bg-base-700/80 p-1.5 rounded border border-white/5">
                      ...
                      <div className="flex items-center gap-2">
                        <a href="/docs/Projet6/Rhouma_Walid_1_config_equipements_082024.pdf" target="_blank" rel="noreferrer" className="text-slate-400 hover:text-cyber-blue" title="Consulter"><Eye className="w-3.5 h-3.5" /></a>
                        <a href="/docs/Projet6/Rhouma_Walid_1_config_equipements_082024.pdf" download className="text-slate-400 hover:text-cyber-blue" title="Télécharger"><Download className="w-3.5 h-3.5" /></a>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="p-3 rounded-xl bg-base-900/60 border border-white/5 space-y-2">
                  ...
                  <div className="space-y-1 text-[11px]">
                    <div className="flex items-center justify-between bg-base-700/80 p-1.5 rounded border border-white/5">
                      ...
                      <div className="flex items-center gap-2">
                        <a href="/docs/Projet6/Rhouma_Walid_2_maquette_packet_tracer.pkt" download className="text-slate-400 hover:text-cyber-emerald" title="Télécharger le fichier PKT"><Download className="w-3.5 h-3.5" /></a>
                      </div>
                    </div>
                  </div>
                </div>
            ...
            <div className="bg-gradient-to-br from-base-700/60 via-base-700/40 to-cyber-emerald/10 border border-cyber-emerald/30 rounded-2xl p-6 backdrop-blur-sm space-y-4 shadow-lg shadow-glow">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-cyber-emerald font-medium">Soutenance orale (30 min)</p>
                </div>
              </div>
              ...
              <div className="space-y-2 text-xs text-slate-300">
                ...
                <div className="p-3 rounded-lg bg-base-900/60 border border-white/5 space-y-1.5">
                  <span className="font-semibold text-cyber-emerald block">Remarques sur l'évaluation :</span>
      ...
      <EvaluationJury
        evaluator={"David Daussin"}
        summary={"Soutenance orale de 30 minutes."}
        quote={"La soutenance est fluide et respecte le temps imparti. Le deroulé est coherent, logique et dynamique. L ensemble est tres structure et didactique. Les reponses aux questions sont pertinentes."}
      />
      <ProjectFooterNav current={6} />
    </div>
  );
}