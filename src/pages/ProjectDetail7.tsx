      ...
      <div className="container-page py-12 lg:py-16 space-y-12">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="p-5 rounded-2xl bg-base-700/60 border border-white/5 backdrop-blur-sm space-y-1">
            <h3 className="text-lg font-bold text-cyber-blue">7 Sondes Nagios</h3>
            <p className="text-xs text-slate-400">RAM, CPU, Disque, Web, BDD...</p>
          </div>
          <div className="p-5 rounded-2xl bg-base-700/60 border border-white/5 backdrop-blur-sm space-y-1">
            <h3 className="text-lg font-bold text-cyber-emerald">RSyslog Centralisé</h3>
            <p className="text-xs text-slate-400">Serveur Web ➔ Serveur Nagios</p>
          </div>
          <div className="p-5 rounded-2xl bg-base-700/60 border border-white/5 backdrop-blur-sm space-y-1">
            <h3 className="text-lg font-bold text-cyber-blue">Doc & Remédiation</h3>
            <p className="text-xs text-slate-400">Commandes Linux & fiches réflexes</p>
          </div>
          <div className="p-5 rounded-2xl bg-base-700/60 border border-white/5 backdrop-blur-sm space-y-1">
            <h3 className="text-lg font-bold text-cyber-emerald">Projet Validé</h3>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <div className="bg-base-700/60 border border-white/5 rounded-2xl p-6 md:p-8 backdrop-blur-sm space-y-6">
              ...
              <div className="p-4 rounded-xl bg-base-900/70 border border-white/5 space-y-3">
                <div className="flex items-center justify-between border-b border-white/5 pb-2 text-xs text-slate-400 font-mono">
            ...
            <div className="bg-base-700/60 border border-white/5 rounded-2xl p-6 md:p-8 backdrop-blur-sm space-y-6">
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-white/5 pb-4">
                ...
                <div className="flex flex-wrap gap-1 bg-base-900/80 p-1 rounded-xl border border-white/5 text-xs font-mono">
                  <button 
                    onClick={() => setActiveTab('cli')}
                    className={`px-3 py-1.5 rounded-lg transition-all ${activeTab === 'cli' ? 'bg-cyber-blue/20 text-cyber-blue border border-cyber-blue/30 font-semibold' : 'text-slate-400 hover:text-white'}`}
                  >
                    Tests Sondes CLI
                  </button>
                  <button 
                    onClick={() => setActiveTab('nagios')}
                    className={`px-3 py-1.5 rounded-lg transition-all ${activeTab === 'nagios' ? 'bg-cyber-blue/20 text-cyber-blue border border-cyber-blue/30 font-semibold' : 'text-slate-400 hover:text-white'}`}
                  >
                    Nagios (2 Hôtes)
                  </button>
                  <button 
                    onClick={() => setActiveTab('logs')}
                    className={`px-3 py-1.5 rounded-lg transition-all ${activeTab === 'logs' ? 'bg-cyber-blue/20 text-cyber-blue border border-cyber-blue/30 font-semibold' : 'text-slate-400 hover:text-white'}`}
                  >
                    RSyslog Config
                  </button>
                </div>
              ...
              {activeTab === 'cli' && (
                <div className="p-4 rounded-xl bg-base-900/60 border border-white/5 space-y-4">
                  <div className="flex items-center justify-between text-xs font-bold text-cyber-blue font-mono border-b border-white/5 pb-2">
                  ...
                  <div className="bg-base-900 rounded-lg p-4 font-mono text-xs border border-white/5 text-slate-300 space-y-5 max-h-[480px] overflow-y-auto">
                    ...
                    <div className="space-y-1.5">
                      ...
                      <div className="pl-3 border-l-2 border-cyber-blue/40 space-y-1 text-cyber-blue">
              ...
              {activeTab === 'nagios' && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {galleryTabs.nagios.map((img, idx) => (
                    <div key={idx} className="p-4 rounded-xl bg-base-900/60 border border-white/5 space-y-3 md:col-span-2">
                      <div className="flex items-center justify-between text-xs font-bold text-cyber-blue font-mono">
                        <span className="flex items-center gap-2">
                      ...
                      <div className="rounded-lg overflow-hidden border border-white/5 bg-base-900 p-1 flex items-center justify-center min-h-[200px]">
                        <img 
                          src={img.src} 
                          alt={img.title} 
              ...
              {activeTab === 'logs' && (
                <div className="p-4 rounded-xl bg-base-900/60 border border-white/5 space-y-4">
                  <div className="flex items-center justify-between text-xs font-bold text-cyber-emerald font-mono">
                    <span className="flex items-center gap-2">
                      <FileText className="w-4 h-4 text-cyber-emerald" /> Centralisation des journaux Apache via RSyslog
                    </span>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 font-mono text-xs">
                    ...
                    <div className="bg-base-900 border border-white/5 rounded-lg p-3 space-y-2">
                      <div className="text-slate-400 text-[11px] font-bold border-b border-white/5 pb-1 flex justify-between">
                        <span>/etc/rsyslog.d/apache.conf (Client)</span>
                        <span className="text-cyber-emerald">192.168.1.13</span>
                      </div>
...
local7.* @192.168.1.15:514`}
                    ...
                    <div className="bg-base-900 border border-white/5 rounded-lg p-3 space-y-2">
                      <div className="text-slate-400 text-[11px] font-bold border-b border-white/5 pb-1 flex justify-between">
                        ...
                        <span className="text-cyber-blue">192.168.1.15</span>
                      </div>
                      <pre className="text-slate-300 leading-relaxed overflow-x-auto text-[11px]">
...
& ~`}
                  ...
                  <div className="bg-base-900 border border-white/5 rounded-lg p-3 font-mono text-[11px] space-y-1">
                  ...
                  <tbody className="divide-y divide-white/5">
                    {monitoringMatrix.map((row, index) => (
                      <tr key={index} className="hover:bg-base-700/30 transition-colors">
                        <td className="py-3 px-4 font-semibold text-cyber-blue font-mono">{row.metric}</td>
                  ...
                  <div className="space-y-1 text-[11px]">
                    <div className="flex items-center justify-between bg-base-700/80 p-1.5 rounded border border-white/5">
                      ...
                      <div className="flex items-center gap-2">
                        <a href="/docs/Projet7/Rhouma_Walid_1_config_nagios_092024.png" target="_blank" rel="noreferrer" className="text-slate-400 hover:text-cyber-blue" title="Consulter"><Eye className="w-3.5 h-3.5" /></a>
                        <a href="/docs/Projet7/Rhouma_Walid_1_config_nagios_092024.png" download className="text-slate-400 hover:text-cyber-blue" title="Télécharger"><Download className="w-3.5 h-3.5" /></a>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="p-3 rounded-xl bg-base-900/60 border border-white/5 space-y-2">
                  ...
                  <div className="space-y-1 text-[11px]">
                    <div className="flex items-center justify-between bg-base-700/80 p-1.5 rounded border border-white/5">
                      ...
                      <div className="flex items-center gap-2">
                        <a href="/docs/Projet7/Rhouma_Walid_2_config_Rsyslog" target="_blank" rel="noreferrer" className="text-slate-400 hover:text-cyber-emerald" title="Consulter"><Eye className="w-3.5 h-3.5" /></a>
                        <a href="/docs/Projet7/Rhouma_Walid_2_config_Rsyslog" download className="text-slate-400 hover:text-cyber-emerald" title="Télécharger"><Download className="w-3.5 h-3.5" /></a>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="p-3 rounded-xl bg-base-900/60 border border-white/5 space-y-2">
                  ...
                  <div className="space-y-1 text-[11px]">
                    <div className="flex items-center justify-between bg-base-700/80 p-1.5 rounded border border-white/5">
                      ...
                      <div className="flex items-center gap-2">
                        <a href="/docs/Projet7/Rhouma_Walid_4_documentation_092024.pdf" target="_blank" rel="noreferrer" className="text-slate-400 hover:text-cyber-blue" title="Consulter"><Eye className="w-3.5 h-3.5" /></a>
                        <a href="/docs/Projet7/Rhouma_Walid_4_documentation_092024.pdf" download className="text-slate-400 hover:text-cyber-blue" title="Télécharger"><Download className="w-3.5 h-3.5" /></a>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <div className="space-y-3">
                {tools.map((item, i) => (
                  <div key={i} className="p-3 rounded-xl bg-base-900/50 border border-white/5 flex items-center justify-between hover:border-white/10 transition-colors">
                    <div className="flex items-center gap-3">
                      <div className="w-9 h-9 p-2 rounded-lg bg-base-700 border border-white/5 shrink-0 flex items-center justify-center">
                        <img 
                          src={item.icon} 
                          alt={item.name} 
              ...
              <div className="space-y-2 text-xs text-slate-300">
                <div className="p-3 rounded-lg bg-base-900/60 border border-white/5 space-y-1.5">
                  <div className="flex items-center justify-between text-[11px]">
                    ...
                    <span className="text-cyber-emerald font-medium">Projet Validé</span>
                  </div>
      ...
      <EvaluationJury
        evaluator={"Julien Drouvroy"}
        summary={"Soutenance de validation."}
        quote={"Tres bonne soutenance. L ensemble des sondes ont ete mises en place ainsi que rsyslog et la documentation associee. Les reponses aux questions sont correctes. Bonne continuation pour la suite."}
      />
      <ProjectFooterNav current={7} />
    </div>
  );
}